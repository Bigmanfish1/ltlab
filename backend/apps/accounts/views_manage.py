...
def _back_to_roles(request):
    """Redirect to the roles list, keeping the page the change was made from."""
    url = reverse("manage_users")
    page = request.POST.get("page", "")
    return redirect(f"{url}?page={page}" if page.isdigit() else url)
...
def manage_users(request):
    ...
    return render(request, "manage/teacher_users.html", {"page": page})
